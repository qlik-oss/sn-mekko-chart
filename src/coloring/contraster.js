...
function luminance(colStr) {
  ...
  if ((m = rHex.exec(colStr) || rHexShort.exec(colStr))) {
    ...
    (m = /^rgb\(\s*(\d{1,3})\s*,\s*(\d{1,3})\s*,\s*(\d{1,3})\s*\)$/i.exec(
      colStr
    ))
  ) {
  ...
  return {
    getBestContrastColor(colorString) {
      if (!cache[colorString]) {
        ...
        const lightContrast =
          (Math.max(L, lightColorLuminance) + 0.05) /
          (Math.min(L, lightColorLuminance) + 0.05);
        const darkContrast =
          (Math.max(L, darkColorLuminance) + 0.05) /
          (Math.min(L, darkColorLuminance) + 0.05);
