function action(e, {
  ...
  if (target.hasAttribute('data-action') && target.hasAttribute('data-component-key')) { // clicked on one of the buttons
    e.preventDefault();
    chart.component(target.getAttribute('data-component-key')).emit(target.getAttribute('data-action'));
    return;
  }

  if (target.childNodes[0] && target.childNodes[0].nodeName === 'BUTTON') { // assume clicked on navigation area
    e.preventDefault();
  }
}
